...
fn chain_is_deterministic_across_repeated_calls() {
    let a = erode(CHAIN_SEED, CHAIN_HMAX, W4_CHAIN_DIM, true, false, false, false, true, 100);
    let b = erode(CHAIN_SEED, CHAIN_HMAX, W4_CHAIN_DIM, true, false, false, false, true, 100);
...
fn erosion_is_thread_count_independent_1_vs_n() {
    let baseline = erode(CHAIN_SEED, CHAIN_HMAX, W4_CHAIN_DIM, true, false, false, false, true, 100);

    const N: usize = 4;
    let handles: Vec<_> = (0..N)
        .map(|_| std::thread::spawn(|| erode(CHAIN_SEED, CHAIN_HMAX, W4_CHAIN_DIM, true, false, false, false, true, 100)))
        .collect();
    for h in handles {
...
fn erosion_conserves_sediment_at_prod_scale() {
    let state = erode(CHAIN_SEED, CHAIN_HMAX, W4_CHAIN_DIM, true, false, false, false, true, 100);
