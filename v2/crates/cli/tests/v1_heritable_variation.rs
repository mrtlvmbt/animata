...
fn mutant_sample(founder: &Genome, stream: u64) -> (bool, Option<CellType>) {
    let econ = phase2_config(SEED).econ;
    let mut child = founder.mutate(stream, 2, false, 0, false, false, false, false, false, false, 0, 0, 0, 0, econ.gdev_cap, false);
...
fn v1_no_mutation_child_inherits_parents_exact_fate() {
    ...
    for stream in [0x1234_5678u64, 0xDEAD_BEEF, 7, 999] {
        let child = no_mutate_parent.mutate(stream, 2, false, 0, false, false, false, false, false, false, 0, 0, 0, 0, econ.gdev_cap, false);
        assert_eq!(
