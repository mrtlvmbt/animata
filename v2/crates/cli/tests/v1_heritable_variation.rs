...
use sim_core::{CellType, Genome, MutationGates};
use std::sync::Arc;
...
fn mutant_sample(founder: &Genome, stream: u64) -> (bool, Option<CellType>) {
    let econ = phase2_config(SEED).econ;
    let mut child = founder.mutate(stream, 2, 0, 0, 0, 0, 0, econ.gdev_cap, &MutationGates { has_light: false, has_predation: false, enable_variable_length: false, evolve_body_size: false, enable_oxygen: false, has_ambient_tolerance: false, enable_mutation_load: false, enable_body_plan: false, enable_propagule: false, enable_provision: false, provision_rate_locked: false, n_propagule_locked: false });
...
fn v1_no_mutation_child_inherits_parents_exact_fate() {
    ...
    for stream in [0x1234_5678u64, 0xDEAD_BEEF, 7, 999] {
        let child = no_mutate_parent.mutate(stream, 2, 0, 0, 0, 0, 0, econ.gdev_cap, &MutationGates { has_light: false, has_predation: false, enable_variable_length: false, evolve_body_size: false, enable_oxygen: false, has_ambient_tolerance: false, enable_mutation_load: false, enable_body_plan: false, enable_propagule: false, enable_provision: false, provision_rate_locked: false, n_propagule_locked: false });
        assert_eq!(
