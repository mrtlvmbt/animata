...
/// Build the whole `world_dim × world_dim` cube terrain as raw chunks (vertices/indices only).
...
/// Build one raw row-band chunk of cube terrain (no Mesh, just raw buffers).
fn build_raw_chunk(
    world_dim: i64,
    world: &dyn WorldView,
    row0: i64,
    ...
    for local_row in 0..band_rows {
        ...
        for col in 0..world_dim {
            ...
            for (top_a, top_b, edge_normal) in edge_configs {
                // Determine the neighbour (col, row) for this edge
                let (ncol, nrow) = neighbors[{
                    if top_a.x < cx {
                        0 // West
                    } else if top_a.x > cx {
                        1 // East
